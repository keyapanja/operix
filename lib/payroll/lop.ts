...
import { isWorkingDay, parseWorkWeek } from "@/lib/leave/work-week";
...
// which the apply flow already enforces). Non-working days (per the company
// work-week) and company holidays inside a leave span are paid and excluded.
...
export async function computeLop(
  ...
  for (const lv of leaves) {
    ...
    while (d.getTime() <= end.getTime()) {
      ...
      if (isWorkingDay(iso, ww, holidaySet)) working += 1;
